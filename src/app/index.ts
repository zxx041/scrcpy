...
const mainProc = async function(hash: string): Promise<void> {
    ...
    // console.log('HotTracker start... ',
    //     'action:', parsedQuery.get('action'),
    //     'uuid:', parsedQuery.get('udid'),
    //     'ws:', parsedQuery.get('ws'),
    //     'remote:', parsedQuery.get('remote')
    // );
...
// @ts-ignore
window.__zcxWsScrcpy_init = async function(hash): Promise<void> {
    await mainProc(hash);
};
